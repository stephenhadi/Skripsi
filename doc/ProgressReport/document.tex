...
\newcommand{\tanggal}{10/01/2021} %Tanggal pembuatan dokumen
...
\begin{enumerate}
	...
	\begin{itemize}
		...
		\item \textit{Table} \\
		Kelas \texttt{table} adalah kelas yang membantu pembuatan \textit{table}.
		
		\item \textit{Pagination} \\
		\textit{Pagination} dibuat menggunakan \texttt{HTML list} yang berada didalam \texttt{<nav>} untuk mengidentifikasikan \textit{pagination} sebagai bagian navigasi kepada pengguna.
	...
	\begin{itemize}
		...
		\begin{figure}[H]
			\centering
			\includegraphics[scale=0.6]{cetaktranskripcancel} 
		...
		\begin{figure}[H]
			\centering
			\includegraphics[scale=0.6]{perubahankuliahcancel} 
